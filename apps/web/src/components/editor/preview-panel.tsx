...
export function PreviewPanel() {
  ...
  const mediaById = useMemo(
    () => new Map(mediaFiles.map((media) => [media.id, media] as const)),
    [mediaFiles]
  );

  const timelineAudioElements = useMemo(() => {
    const items: Array<{
      key: string;
      kind: "audio" | "video";
      src: string;
    ...
    for (const track of tracks) {
      for (const element of track.elements) {
        ...
        if (element.mediaId === "test") continue;
        const media = mediaById.get(element.mediaId);
        if (!media) continue;
        if (media.type !== "audio" && media.type !== "video") continue;
        ...
        items.push({
          ...
          muted: !!track.muted || !!element.muted,
        });
      }
    }
  ...
  // Render an element (canvas handles visuals now). Audio playback runs through hidden media elements.
  ...
  return (
    <>
      {timelineAudioElements.map((entry) =>
        entry.kind === "audio" ? (
          <AudioPlayer
            key={entry.key}
            src={entry.src}
