...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'analyze') {
      ...
      const result = await twelveLabsService.analyzeVideo(userId, videoUrl, language);
      ...
      const result = await twelveLabsService.searchVideos(userId, query, searchOptions);
