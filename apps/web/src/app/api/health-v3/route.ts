  ...
  try {
    ...
    if (hasSupabase) {
      try {
        ...
        await supabaseSelectOne("media_twelvelabs", {
        ...
        if (
          checks.supabaseMessage.includes(
            'relation "media_twelvelabs" does not exist'
          )
        ) {
          checks.supabaseMessage =
            "Connected, but media_twelvelabs table not created yet. Please run the SQL schema.";
    ...
    if (hasTwelvelabs) {
      try {
        ...
        checks.twelvelabsMessage =
          twelvelabsError instanceof Error
            ? twelvelabsError.message
            : "Connection failed";
      }
    }
